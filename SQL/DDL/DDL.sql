...
-- GRANT ALL PRIVILEGES ON project.* TO `beyond`@`%`;
-- SHOW GRANTS FOR `beyond`@`%`;
-- FLUSH PRIVILEGES;

-- 1단계: 최하위 자식 테이블 (다른 테이블을 참조만 하는 말단들)
DROP TABLE IF EXISTS user_notification;         -- notification, users 참조
DROP TABLE IF EXISTS comments;                  -- posts, users 참조
DROP TABLE IF EXISTS news_summary;              -- News, Chatbot 참조
DROP TABLE IF EXISTS portfolio_analysis;        -- Chatbot 참조
DROP TABLE IF EXISTS stock_summary;             -- Chatbot 참조
DROP TABLE IF EXISTS watchlist;                 -- users, stocks 참조
DROP TABLE IF EXISTS portfolios;                -- accounts, stocks 참조
DROP TABLE IF EXISTS trades;                    -- accounts, stocks 참조
DROP TABLE IF EXISTS stock_price;               -- stocks 참조
DROP TABLE IF EXISTS deposit_withdraw_history;  -- accounts 참조
DROP TABLE IF EXISTS loan_repayment_history;    -- loan, schedule 참조
DROP TABLE IF EXISTS loan_repayment_schedule;   -- loan 참조
DROP TABLE IF EXISTS admin_notification;        -- users, admins 참조
DROP TABLE IF EXISTS admin_modification_log;    -- admins 참조


-- 2단계: 중간 관계 테이블 (참조를 받으면서 동시에 누군가를 참조함)
DROP TABLE IF EXISTS posts;                     -- users 참조 (comments의 부모)
DROP TABLE IF EXISTS notification;              -- (user_notification의 부모)
DROP TABLE IF EXISTS chatbot;                   -- (summary, analysis의 부모)
DROP TABLE IF EXISTS news;                      -- (news_summary의 부모)
DROP TABLE IF EXISTS loan;                      -- accounts 참조 (history, schedule의 부모)

-- 3단계: 최상위 부모 테이블 (가장 기본이 되는 테이블들)
DROP TABLE IF EXISTS login;                     -- users 참조
DROP TABLE IF EXISTS accounts;                  -- users 참조
DROP TABLE IF EXISTS stocks;                    -- (포트폴리오, 거래 등의 기준)
DROP TABLE IF EXISTS loan_policy;               -- 독립적 테이블
DROP TABLE IF EXISTS dashboard;                 -- 독립적 테이블
DROP TABLE IF EXISTS admin_credentials;         -- admins 참조
DROP TABLE IF EXISTS admins;                    -- 관리자 테이블 기준
DROP TABLE IF EXISTS users;                     -- 모든 서비스의 핵심 (가장 마지막에 삭제)
...
CREATE TABLE `Chatbot` (
    ...
    -- users 테이블의 ID 컬럼명에 맞춰 확인 필요 (예: login 테이블의 ID 참조 시)
...
CREATE TABLE `News` (
    ...
    `URL` VARCHAR(255) NOT NULL COMMENT 'URL', -- FK 참조를 위해 VARCHAR로 설정 및 길이 제한
    ...
    CONSTRAINT `uq_news_url` UNIQUE (`URL`), -- News_summary에서 참조하기 위한 유니크 제약
